...
import { Plus, Trash2, ChevronLeft, Image as ImageIcon, X, Loader2 } from "lucide-react";
...
const TEST_TYPES = ["DTM", "SAT", "MILLIY_SERTIFIKAT", "ATTESTATSIYA", "DTM_GROUP", "SAT_GROUP", "MILLIY_GROUP", "ATTESTATSIYA_GROUP"];
...
export default function CreateTestPage() {
  ...
  const handleNextStep = async () => {
    if (createdTestId) { setStep("questions"); return; }
    const input: any = {
    ...
    if (testInfo.testAccess === "GROUP" && testInfo.groupId) input.groupId = testInfo.groupId;
  ...
  const uploadImage = async (file: File, uid: string) => {
    setQuestions((qs) => qs.map((q) => q.uid === uid ? { ...q, uploading: true } : q));
    try {
      ...
      if (data.url) {
        setQuestions((qs) => qs.map((q) => q.uid === uid ? { ...q, questionImage: data.url, uploading: false } : q));
      }
    } catch {
      toast.error("Rasm yuklanmadi");
      setQuestions((qs) => qs.map((q) => q.uid === uid ? { ...q, uploading: false } : q));
    }
  };
  ...
  const updateQ = (uid: string, field: keyof QuestionDraft, value: any) =>
    setQuestions((qs) => qs.map((q) => q.uid === uid ? { ...q, [field]: value } : q));
        ...
        {(["info", "questions"] as const).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
              step === s ? "bg-primary text-white" : createdTestId && s === "questions" ? "bg-green-500 text-white" : "bg-muted text-muted-foreground"
            }`}>{i + 1}</div>
        ...
        <div className="bg-background rounded-2xl border border-border p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background"
                ...
                {TEST_TYPES.map((t) => <option key={t} value={t}>{t.replace("_GROUP", " (Guruh)")}</option>)}
              </select>
            </div>
            <div>
          ...
          {testInfo.testAccess === "GROUP" && (
            <div>
              ...
              <Input placeholder="Guruh ID" value={testInfo.groupId}
          ...
          <div>
            ...
            <textarea className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background resize-none"
              rows={3} placeholder="Test haqida qisqacha..."
              value={testInfo.testDesc} onChange={(e) => setTestInfo({ ...testInfo, testDesc: e.target.value })} />
          </div>

          <button onClick={handleNextStep} disabled={!testInfo.testTitle || creating}
        ...
        <div className="space-y-4">
          {questions.map((q, qIndex) => (
            <QuestionCard
              ...
              onRemove={() => setQuestions((qs) => qs.filter((x) => x.uid !== q.uid))}
              onImagePick={(file) => uploadImage(file, q.uid)}
              canRemove={questions.length > 1}
            />
          ))}

          <button onClick={() => setQuestions((qs) => [...qs, newQuestion(qs.length)])}
...
function QuestionCard({ q, index, onUpdate, onUpdateOption, onRemove, onImagePick, canRemove }: {
  q: QuestionDraft;
  index: number;
