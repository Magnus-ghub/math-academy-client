...
import { X, Search, UserPlus, Check, Loader2, UserMinus, Users } from "lucide-react";
...
export default function AddUserToGroupModal({ group, onClose, onAdded }: Props) {
  ...
  const { data: membersData, loading: membersLoading, refetch: refetchMembers } =
    useQuery<{ getGroupMembers: any[] }>(GET_GROUP_MEMBERS, {
      variables: { groupId: group?.id },
      skip: !group,
      fetchPolicy: "network-only",
    });

  const { data: memberIdsData, refetch: refetchIds } =
    useQuery<{ getGroupMemberIds: string[] }>(GET_GROUP_MEMBER_IDS, {
      variables: { groupId: group?.id },
      skip: !group,
      fetchPolicy: "network-only",
    });
  ...
  const existingMemberIds = new Set([
    ...(memberIdsData?.getGroupMemberIds ?? []),
    ...addedIds,
  ]);
  ...
  const [removeUser] = useMutation(REMOVE_USER_FROM_GROUP, {
    onCompleted: () => {
      ...
      refetchIds();
      onAdded();
      toast.success("Foydalanuvchi guruhdan chiqarildi");
    },
    onError: () => { setRemovingId(null); toast.error("Xatolik yuz berdi"); },
  });
        ...
        <div className="flex items-center justify-between p-5 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {members.length} a'zo · {group.durationMonths} oy
            </p>
          </div>
            ...
            {membersLoading ? (
              ...
              <ul className="divide-y divide-border">
                {members.map((member: any) => (
                  <li key={member.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Tugashi: {new Date(member.expiresAt).toLocaleDateString("uz-UZ")}
                      </p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${roleColors[member.userRole]}`}>
                      {roleLabels[member.userRole] || member.userRole}
                    </span>
                    <button
                      onClick={() => {
                        setRemovingId(member.id);
                        removeUser({ variables: { groupId: group.id, userId: member.id } });
                      }}
                      disabled={removingId === member.id}
                      className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 disabled:opacity-40 transition-colors shrink-0"
